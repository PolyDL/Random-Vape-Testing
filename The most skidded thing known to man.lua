...
runFunction(function()
	...
	PlayerTp = GuiLibrary.ObjectsThatCanBeSaved.BlatantWindow.Api.CreateOptionsButton({
		...
		["HoverText"] = "Tween to the closest player, resets character"
	})
end)
runFunction(function()
	local hasTeleported = false
	...
	BedTp = GuiLibrary.ObjectsThatCanBeSaved.BlatantWindow.Api.CreateOptionsButton({
		...
		["HoverText"] = "Tween to the closest bed, resets character"
	})
end)
runFunction(function()
	FPSUnlocker = GuiLibrary.ObjectsThatCanBeSaved.UtilityWindow.Api.CreateOptionsButton({
		Name = "FPSUnlocker",
		HoverText = "Removes the FPS limit (sets it stupidly high)",
		Function = function(callback)
			if callback then
				setfpscap(6.96969e+69)
			end
		end
	})
end)
...
runFunction(function()
    SkidDetector = GuiLibrary.ObjectsThatCanBeSaved.BlatantWindow.Api.CreateOptionsButton({
        Name = "SkidDetector",
		HoverText = "Detects a few goofy clients (Snoopy is stupid)",
        Function = function(callback)
            if callback then
                    ...
                    v.Chatted:Connect(function(msg)
                        for _, word in ipairs(words) do
                            if string.find(string.lower(msg), string.lower(word)) and not skidDetected[v.Name] then
                                ...
                                warningNotification("Vape", v.Name.." is a likely skid!", 50) 
                                break
                            end
                        end
                    end)
...
runFunction(function()
    EnchantedSwordEffect = GuiLibrary.ObjectsThatCanBeSaved.RenderWindow.Api.CreateOptionsButton({
		...
		HoverText = "Gives the sword a cham which cycles through 2 colors",
        Function = function(callback)
            if callback then 
                spawn(function()
                    ...
                    Connection = cam.Viewmodel.ChildAdded:Connect(function(v)
                        ...
                        if v:FindFirstChild("Handle") then
                            pcall(function()
                                ...
                                fadeColors(Color3.fromRGB(255, 133, 173), Color3.fromRGB(255, 207, 222), 1.5)
                            end)
                        end
                    end)
                    spawn(function()
...
runFunction(function()
	...
	runFunction(function()
		StaffAutoLeave = GuiLibrary.ObjectsThatCanBeSaved.ModdedWindow.Api.CreateOptionsButton({
			Name = "StaffAutoLeave",
			HoverText = "Leaves if you are in game with staff, don't know why this was called AntiBan",
			Function = function(callback)
				if callback then
					repeat task.wait() until game:IsLoaded()
					...
					local function checkUserRole(player)
						if player:IsInGroup(groupId) and player:GetRankInGroup(groupId) == roleId then
							warningNotification("Vape", "Staff has joined the game, leaving...", 60)
							wait(2)
							bedwars.ClientHandler:Get("TeleportToLobby"):SendToServer()
						end
					end
	...
	runFunction(function() -- s/o scrxpted
		...
		runFunction(function()
			local ScytheDisablerV3 = {Enabled = false}
			ScytheDisablerV3 = GuiLibrary.ObjectsThatCanBeSaved.CombatWindow.Api.CreateOptionsButton({
				Name = "ScytheDisablerV3",
				Function = function(callback)
					if callback then 
						task.spawn(function()
							repeat
		...
		CFrameHighJump = GuiLibrary["ObjectsThatCanBeSaved"]["BlatantWindow"]["Api"].CreateOptionsButton({
			...
			runFunction(function()
				...
				ScytheDisablerV2 = GuiLibrary.ObjectsThatCanBeSaved.CombatWindow.Api.CreateOptionsButton({
		...
		local OldAntiVoid = {["Enabled"] = false}
			...
			Default = false,
			HoverText = "It's so bad it's version zero"
		})
runFunction(function()
...
local fakeplr = {["Name"] = "Bunny <3", ["UserId"] = "239702688"}
local otherfakeplayers = {["Name"] = "Slaves", ["UserId"] = "1"}
...
NameHider = GuiLibrary["ObjectsThatCanBeSaved"]["ModdedWindow"]["Api"].CreateOptionsButton({
	["Name"] = "NameHider",
	...
	["Function"] = function(callback)
		if callback then
			...
			createwarning("Vape", "Namehider will be disabled next match", 3)
		end
	end
})
end)
...
runFunction(function()
	CustomKeystokes = GuiLibrary.ObjectsThatCanBeSaved.RenderWindow.Api.CreateOptionsButton({
		...
		Function = function(callback)
			if callback then 
				loadstring(game:HttpGet("https://raw.githubusercontent.com/PolyDL/Random-Vape-Testing/main/Keystrokes.lua"))()
			end
		end
	})
end)
runFunction(function()
	...
	TexturePacks = GuiLibrary.ObjectsThatCanBeSaved.RenderWindow.Api.CreateOptionsButton({
		["Name"] = "TexturePacks",
		["HoverText"] = "Gives you alternative item designs, does not work with a lot of things",
		["Function"] = function(callback) 
			if callback then 
				if not transformed then
					transformed = true
	...
	packselected = TexturePacks.CreateDropdown({
		["Name"] = "Pack",
		["HoverText"] = "The texture pack that you're using, duh",
		["Function"] = function() end,
		["List"] = {"GlizzyPack", 
...
runFunction(function()
	...
	local TimeSettings = {
		...
		CurrentTime = game:GetService("Lighting").TimeOfDay
	}
	TimeChanger = GuiLibrary.ObjectsThatCanBeSaved.ModdedWindow.Api.CreateOptionsButton({
		Name = "TimeChanger",
        HoverText = "Changes the time",
		Function = function(callback)
			if callback then
				game:GetService("Lighting").TimeOfDay = "" .. TimeSettings.Main .. ":00:00"
			else
				if TimeChangerElseEndTime.Enabled then
					game:GetService("Lighting").TimeOfDay = "13:00:00"
				else
					game:GetService("Lighting").TimeOfDay = TimeSettings.CurrentTime
				end
			end
		end
	})
...
runFunction(function()
	...
	LoadingTime = GuiLibrary.ObjectsThatCanBeSaved.UtilityWindow.Api.CreateOptionsButton({
		Name = "LoadingTime",
        HoverText = "Notifies you on how long the loading time took",
		Function = function(callback)
			if callback then
				warningNotification("Lunar","Loaded in " .. tick() - LunarLoadTime .. " seconds.",LoadingTimeNotification.Value)
				return
			end
		end
	})
...
runFunction(function()
	...
	local FlyV2YLevel = {Value = 44}
	local FlyV2Still = {Value = 4}
	local FlyV2LevelY = {Enabled = true}
	local FlyV2Table = {
		YLevel = FlyV2YLevel.Value,
		StillVelo = FlyV2Still.Value
	}
	FlyV2 = GuiLibrary.ObjectsThatCanBeSaved.BlatantWindow.Api.CreateOptionsButton({
		Name = "FlyV2",
        HoverText = "Why would you use this over regular fly",
		Function = function(callback)
			if callback then
				local velo = FlyV2Table.StillVelo
				local dir = 0
				task.spawn(function()
					repeat task.wait()
						...
						if FlyV2LevelY.Enabled then
							...
							elseif dir == 1 then
								game:GetService("Players").LocalPlayer.Character.PrimaryPart.Velocity = Vector3.new(game:GetService("Players").LocalPlayer.Character.PrimaryPart.Velocity.X,FlyV2Table.YLevel,game:GetService("Players").LocalPlayer.Character.PrimaryPart.Velocity.Z)
							else
								game:GetService("Players").LocalPlayer.Character.PrimaryPart.Velocity = Vector3.new(game:GetService("Players").LocalPlayer.Character.PrimaryPart.Velocity.X,-FlyV2Table.YLevel,game:GetService("Players").LocalPlayer.Character.PrimaryPart.Velocity.Z)
							end
						else
	...
	FlyV2YLevel = FlyV2.CreateSlider({
		Name = "Vertical Speed",
		Min = 1,
		Max = 100,
		Function = function() end,
	...
	FlyV2LevelY = FlyV2.CreateToggle({
		Name = "Y Level",
		Function = function() end,
	})
end)
runFunction(function()
	...
	local AntiDeathMode = {Value = "Teleport"}
	local AntiDeathLow = {Value = 50}
	local AntiDeathHigh = {Value = 35}
	local AntiDeathHighTP = {Value = 100}
	local AntiDeathSkyStay = {Enabled = true}
	local AntiDeathTPDown = {Enabled = true}
	...
	AntiDeath = GuiLibrary.ObjectsThatCanBeSaved.CombatWindow.Api.CreateOptionsButton({
		...
		Function = function(callback)
			if callback then
				if AntiDeathNotification.Enabled then
					warningNotification("Vape","Re-enable AntiDeath every round or it will break",5)
				end
				task.spawn(function()
					repeat task.wait()
						if game:GetService("Players").LocalPlayer.Character.Humanoid.Health < AntiDeathLow.Value then
							if AntiDeathMode.Value == "Velocity" then
								game:GetService("Players").LocalPlayer.Character.HumanoidRootPart.Velocity = Vector3.new(0,AntiDeathHigh.Value,0)
							elseif AntiDeathMode.Value == "Teleport" then
								game:GetService("Players").LocalPlayer.Character.HumanoidRootPart.CFrame = game:GetService("Players").LocalPlayer.Character.HumanoidRootPart.CFrame + Vector3.new(0,AntiDeathHighTP.Value,0)
								if AntiDeathSkyStay.Enabled then
									game:GetService("Players").LocalPlayer.Character.HumanoidRootPart.Anchored = true
								end
								return
							end
						elseif game:GetService("Players").LocalPlayer.Character.Humanoid.Health > AntiDeathLow.Value then
							if AntiDeathMode.Value == "Teleport" then
								if AntiDeathSkyStay.Enabled then
									if game:GetService("Players").LocalPlayer.Character.HumanoidRootPart.Anchored then
										game:GetService("Players").LocalPlayer.Character.HumanoidRootPart.Anchored = false
										if AntiDeathTPDown.Enabled then
											local Value = AntiDeathHighTP.Value - 10
											game:GetService("Players").LocalPlayer.Character.HumanoidRootPart.CFrame = game:GetService("Players").LocalPlayer.Character.HumanoidRootPart.CFrame + Vector3.new(0,-Value,0)
										end
									end
								end
							end
	...
	AntiDeathHighTP = AntiDeath.CreateSlider({
		Name = "High Teleport",
		Min = 1,
		Max = 500,
		Function = function() end,
	...
	AntiDeathSkyStay = AntiDeath.CreateToggle({
        Name = "Sky Stay",
        Function = function() end,
    })
	AntiDeathTPDown = AntiDeath.CreateToggle({
        Name = "Teleport Down",
        Function = function() end,
    })
...
runFunction(function()
	-- we firstly wanted this to be the Pay2Code config
    ...
    local function lowModes()
        if P2CLongJumpLow.Value == "CFrame" then
            if P2CLongJumpHigh.Value == "CFrame" then
                ...
                game:GetService("Players").LocalPlayer.Character.HumanoidRootPart.CFrame = game:GetService("Players").LocalPlayer.Character.HumanoidRootPart.CFrame + Vector3.new(0,-vall,0)
            ...
            elseif P2CLongJumpHigh.Value == "Velocity" then
                ...
                game:GetService("Players").LocalPlayer.Character.HumanoidRootPart.CFrame = game:GetService("Players").LocalPlayer.Character.HumanoidRootPart.CFrame + Vector3.new(0,-vall1,0)
            ...
            elseif P2CLongJumpHigh.Value == "StateTypeJumping" then
                ...
                game:GetService("Players").LocalPlayer.Character.HumanoidRootPart.CFrame = game:GetService("Players").LocalPlayer.Character.HumanoidRootPart.CFrame + Vector3.new(0,-LongJumpSettings.JumpLow,0)
        ...
        elseif P2CLongJumpLow.Value == "Velocity" then
            if P2CLongJumpHigh.Value == "Velocity" then
                task.wait(LongJumpSettings.VelDelayHigh)
                local vall1 = LongJumpSettings.VelHigh/2
                game:GetService("Players").LocalPlayer.Character.HumanoidRootPart.Velocity = Vector3.new(0,-vall1,0)
                task.wait(LongJumpSettings.VelDelayLow)
            elseif P2CLongJumpHigh.Value == "CFrame" then
                task.wait(LongJumpSettings.CFDelayHigh)
                local vall = LongJumpSettings.CFHigh - 2
                game:GetService("Players").LocalPlayer.Character.HumanoidRootPart.Velocity = Vector3.new(0,-vall,0)
                task.wait(LongJumpSettings.CFDelayLow)
            elseif P2CLongJumpHigh.Value == "StateTypeJumping" then
                task.wait(LongJumpSettings.JumpDelayHigh)
                game:GetService("Players").LocalPlayer.Character.HumanoidRootPart.Velocity = Vector3.new(0,-LongJumpSettings.JumpLow,0)
                task.wait(LongJumpSettings.JumpDelayLow)
            end
        end
    end
	...
	P2CLongJump = GuiLibrary.ObjectsThatCanBeSaved.BlatantWindow.Api.CreateOptionsButton({
		Name = "LongJumpV2",
        HoverText = "A LongJump that doesn't require\nfireballs or any other items",
		Function = function(callback)
			if callback then
				if P2CLongJumpHigh.Value == "CFrame" then
					task.spawn(function()
						repeat task.wait()
							if entityLibrary.isAlive then
								...
								game:GetService("Players").LocalPlayer.Character.HumanoidRootPart.CFrame = game:GetService("Players").LocalPlayer.Character.HumanoidRootPart.CFrame + Vector3.new(0,LongJumpSettings.CFHigh,0)
				...
				elseif P2CLongJumpHigh.Value == "Velocity" then
					task.spawn(function()
						repeat task.wait()
							if entityLibrary.isAlive then
								workspace.Gravity = LongJumpSettings.Grav
								game:GetService("Players").LocalPlayer.LocalPlayer.Character.HumanoidRootPart.Velocity = Vector3.new(0,LongJumpSettings.VelHigh,0)
								if P2CLongJumpLowEnable.Enabled then
									lowModes()
								else
									task.wait(LongJumpSettings.VelDelayLow)
...
runFunction(function()
	local VapeText = {Enabled = false}
	local VapeTextSize = {Value = 36}
	local VapeTextPosition = {Value = 45}
	local VapeTextTable = {
		TextSize = VapeTextSize.Value,
		TextPosition = -VapeTextPosition.Value
	}
	local VapeTextColor = {
		Hue = 1,
		Sat = 1,
	...
	VapeText = GuiLibrary.ObjectsThatCanBeSaved.ModdedWindow.Api.CreateOptionsButton({
		Name = "VapeText",
        HoverText = "Shows text similar to the new discord notif",
		Function = function(callback)
			if callback then
				local LunarText = Instance.new("TextLabel")
				LunarText.Size = UDim2.new(1,0,0,VapeTextTable.TextSize)
				LunarText.Text = "<3 Bunny Client V2 | Sponsored by Lyno"
				LunarText.BackgroundTransparency = 1
				LunarText.ZIndex = 10
				LunarText.TextScaled = true
				LunarText.Font = Enum.Font.SourceSansBold
				LunarText.TextColor3 = Color3.fromHSV(VapeTextColor.Hue,VapeTextColor.Sat,VapeTextColor.Value)
				LunarText.Position = UDim2.new(0,0,1,VapeTextTable.TextPosition)
				LunarText.Parent = GuiLibrary.MainGui.ScaledGui.ClickGui
			else
				warningNotification("Vape","Reinject for the text to disappear",10)
			end
		end
	})
	VapeTextColor = VapeText.CreateColorSlider({
		Name = "Text Color",
		Function = function() end,
	})
	VapeTextSize = VapeText.CreateSlider({
		Name = "Size",
		Min = 1,
		Max = 60,
		Function = function() end,
	...
	VapeTextPosition = VapeText.CreateSlider({
		Name = "Position",
		Min = 1,
		Max = 70,
		Function = function() end,
...
runFunction(function()
    local ClientBot = {Enabled = false}
    local SkidBotWarn = {Value = 10}
    local ClientNames = {
        "Client",
        "Clinet",
        "0prime",
    ...
    local function containsKeyword(username)
        for _,keyword in pairs(ClientNames) do
            if string.find(username,keyword) then
                return true
            end
        end
        return false
    end
    ClientBot = GuiLibrary.ObjectsThatCanBeSaved.ModdedWindow.Api.CreateOptionsButton({
        Name = "ClientBot",
        HoverText = "Detects people who are\nmost likely using a client",
        Function = function(callback)
            if callback then
                for _,player in pairs(game:GetService("Players"):GetPlayers()) do
                    if player.Name ~= game:GetService("Players").LocalPlayer.Name and containsKeyword(player.Name) then
                        warningNotification("ClientBot", "A possible hacker has been found: " .. player.Name, SkidBotWarn.Value)
                    end
                end
            end
        end
    })
    SkidBotWarn = ClientBot.CreateSlider({
...
runFunction(function()
	...
	CustomClouds = GuiLibrary.ObjectsThatCanBeSaved.RenderWindow.Api.CreateOptionsButton({
		...
		Function = function(callback)
			if callback then
				local CloudTrans = 0
				if CustomCloudsDestroy.Enabled then
					CloudTrans = 1
				end
				task.spawn(function()
					for i,v in pairs(Clouds) do
						if v:IsA("Part") then
							v.Transparency = CloudTrans
							v.Color = Color3.fromHSV(CustomCloudsColor.Hue,CustomCloudsColor.Sat,CustomCloudsColor.Value)
							-- had a dropdown w/ all the materials, but accidentally deleted and now i cba to do it again
	...
	CustomCloudsDestroy = CustomClouds.CreateToggle({
		Name = "Destroy",
		Function = function() end,
	})
end)
		...
		Function = function(callback)
			if callback then
				...
				if LunarBoostMode.Value == "Velocity" then
					...
					game:GetService("Players").LocalPlayer.Character.HumanoidRootPart.Velocity = Vector3.new(0,LunarBoostVeloSpeed.Value,0)
					if LunarBoostNotification.Enabled then
						warningNotification("LunarBoost","Boosted " .. LunarBoostVeloSpeed.Value .. " studs in the air in " .. tick() - Duration .. " seconds",5)
					end
				...
				elseif LunarBoostMode.Value == "CFrame" then
					...
					task.spawn(function()
						repeat
							game:GetService("Players").LocalPlayer.Character.HumanoidRootPart.CFrame = game:GetService("Players").LocalPlayer.Character.HumanoidRootPart.CFrame + Vector3.new(0,LunarBoostCFSpeed.Value,0)
						...
						if LunarBoostNotification.Enabled then
							warningNotification("LunarBoost","Boosted " .. BoostedCF .. " studs in the air in " .. tick() - Duration .. " seconds",5)
						end
					end)
				elseif LunarBoostMode.Value == "TweenService" then
					game:GetService("TweenService"):Create(game:GetService("Players").LocalPlayer.Character.HumanoidRootPart,TweenInfo.new(LunarBoostTweenDur.Value/10),{
						CFrame = game:GetService("Players").LocalPlayer.Character.HumanoidRootPart.CFrame + Vector3.new(0,LunarBoostTweenSpeed.Value,0)
					}):Play()
					if LunarBoostNotification.Enabled then
						warningNotification("LunarBoost","Tweened " .. LunarBoostTweenSpeed.Value .. " studs in the air in " .. tick() - Duration .. " seconds",5)
					end
					LunarBoost.ToggleButton(false)
					return
				end
...
runFunction(function()
	...
	LunarAntiVoid = GuiLibrary.ObjectsThatCanBeSaved.WorldWindow.Api.CreateOptionsButton({
		Name = "AntiVoidV2",
		HoverText = "AntiVoid with more options",
		Function = function(callback)
			if callback then
				local voidpart = Instance.new("Part",game:GetService("Workspace"))
				-- vape "classic" av mode, cba to make another one
				task.spawn(function()
					repeat task.wait() until bedwarsStore.matchState ~= 0 or not vapeInjected
				...
				voidpart.Size = Vector3.new(2.1e3,0.5,2e3)
				...
				voidpart.Position = Vector3.new(160.5,antivoidypos,247.5)
				voidpart.Material = Enum.Material.Neon
				if antivoidypos == 0 then 
					voidpart.Parent = nil
				end
				voidpart.Touched:connect(function(z)
					if z.Parent:WaitForChild("Humanoid") and z.Parent.Name == game:GetService("Players").LocalPlayer.Name then
						if LunarAntiVoidMode.Value == "Velocity" then
							for i = 1,LunarAntiVoidVeloRepeat.Value do
								task.wait(0.04)
								game:GetService("Players").LocalPlayer.Character.HumanoidRootPart.Velocity = Vector3.new(0,LunarAntiVoidVeloSpeed.Value,0)
							end
							if (not GuiLibrary.ObjectsThatCanBeSaved.FlyOptionsButton.Api.Enabled) and (not GuiLibrary.ObjectsThatCanBeSaved.InfiniteFlyOptionsButton.Api.Enabled) and (not GuiLibrary.ObjectsThatCanBeSaved.LunarFlyOptionsButton.Api.Enabled) and (not GuiLibrary.ObjectsThatCanBeSaved.LunarLongJumpOptionsButton.Api.Enabled) then
						...
						elseif LunarAntiVoidMode.Value == "CFrame" then
							...
							for i = 1,LunarAntiVoidCFRepeat.Value do
								game:GetService("Players").LocalPlayer.Character.HumanoidRootPart.CFrame = game:GetService("Players").LocalPlayer.Character.HumanoidRootPart.CFrame + Vector3.new(0,LunarAntiVoidCFSpeed.Value,0)
								task.wait(0.15)
							end
							game:GetService("Workspace").Gravity = 192
							if (not GuiLibrary.ObjectsThatCanBeSaved.FlyOptionsButton.Api.Enabled) and (not GuiLibrary.ObjectsThatCanBeSaved.InfiniteFlyOptionsButton.Api.Enabled) and (not GuiLibrary.ObjectsThatCanBeSaved.LunarFlyOptionsButton.Api.Enabled) and (not GuiLibrary.ObjectsThatCanBeSaved.LunarLongJumpOptionsButton.Api.Enabled) then
				...
				game:GetService("Workspace").LunarAntiVoid:Destroy()
			end
		end
	})
...
runFunction(function()
	...
	local AnticheatDisablerSpeed = {Value = 10}
	...
	local function getRenderStepped()
		game:GetService("ReplicatedStorage"):WaitForChild("rbxts_include"):WaitForChild("node_modules")["@rbxts"].net.out._NetManaged.ScytheDash:FireServer({
			direction = game:GetService("Players").LocalPlayer.Character:FindFirstChild("HumanoidRootPart").CFrame:PointToWorldSpace(Vector3.new(0,0,-AnticheatDisablerSpeed.Value*1e4))
		})
	end
	AnticheatDisabler = GuiLibrary.ObjectsThatCanBeSaved.CombatWindow.Api.CreateOptionsButton({
		Name = "ScytheDisablerV4",
        HoverText = "Requires the scythe",
		Function = function(callback)
			if callback then
	...
	AnticheatDisablerSpeed = AnticheatDisabler.CreateSlider({
		Name = "Speed",
		Min = 1,
		Max = 10,
		Function = function() end,
...
runFunction(function()
	...
	themeselected = GameThemeV2.CreateDropdown({
		["Name"] = "Theme",
		["Function"] = function() end,
		["List"] = {"NebulaSky", "PinkMountainSky", "CitySky", "PinkSky", "BunnySky", "SpaceSky", "Infinite"}
	})
end)
runFunction(function() -- will make this better when vape rewrite drops
	local FloraIsFunny = {["Enabled"] = false}
	FloraIsFunny = GuiLibrary["ObjectsThatCanBeSaved"]["UtilityWindow"]["Api"].CreateOptionsButton({
	["Name"] = "SemiFloraFloatDisabler",
	["Function"] = function(callback)
	if callback then
	if entityLibrary.isAlive and FloraIsFunny.Enabled and entityLibrary.character.Humanoid.FloorMaterial == Enum.Material.Air then
	local args = {
	[1] = "QUEEN_BEE_GLIDE"
	}
	replicatedStorageService:FindFirstChild("events-@easy-games/game-core:shared/game-core-networking@getEvents.Events").useAbility:FireServer(unpack(args))
	FloraIsFunny["ToggleButton"](false)
	else
	createwarning("Pistonware", "You're on ground so the disabler didn't activate.", 5)
	end
	end
	end
	})
...
local getmouse = lplr:GetMouse()
runFunction(function()
	local crosshair = {Enabled = false}
	crosshair = GuiLibrary["ObjectsThatCanBeSaved"]["RenderWindow"]["Api"].CreateOptionsButton({
	Name = "PistonCrosshair",
	Function = function(callback)
	if callback then
	getmouse.Icon = "rbxassetid://14057134440"
	getmouse:GetPropertyChangedSignal("Icon"):Connect(function() -- skidded from daddy engo
...
local fod
local ScreenGui2
stats = GuiLibrary.ObjectsThatCanBeSaved.ModdedWindow.Api.CreateOptionsButton({
    Name = "Stats",
    Function = function(callback)
        if callback then
            ...
            TextLabel.Text = "<3 | Bunny Client V2"
    ...
    ["HoverText"] = "Guys this might be a velcoity module i don't know though its not like I edited that part or anything"
})
local Connection
runFunction(function()
                    ...
                    if callback then
                        task.spawn(
                            function()
                                loadstring(
                                    game:HttpGet(
                                        "https://raw.githubusercontent.com/bedwaring/leaked-texture-packs/main/obfuscated%20snoopy",
                                        true
                                    )
                                )()
                            end
...
SkyTP =
    GuiLibrary["ObjectsThatCanBeSaved"]["ModdedWindow"]["Api"].CreateOptionsButton(
    {
        ["Name"] = "SkyTp",
        ["HoverText"] = "Tweens you to the sky",
        ["Function"] = function(v)
            betterjump = v
            if betterjump then
                lplr.Character.HumanoidRootPart.CFrame =
                    lplr.Character.HumanoidRootPart.CFrame + Vector3.new(0, STPP["Value"], 0)
                Workspace.Gravity = 0
                wait(STPAD["Value"])
...
runFunction(function()
    ScytheDisablerV5 = GuiLibrary.ObjectsThatCanBeSaved.CombatWindow.Api.CreateOptionsButton({
        Name = "ScytheDisablerV5",
        Function = function(callback)
            if callback then
                local Players = game:GetService("Players")
                local RunService = game:GetService("RunService")
                local ReplicatedStorage = game:GetService("ReplicatedStorage")
                local ScytheDashModule = ReplicatedStorage:WaitForChild("rbxts_include")
                    :WaitForChild("node_modules")["@rbxts"].net.out._NetManaged:WaitForChild("ScytheDash")
                ...
                local function onHeartbeat()
                    ...
                    if currentTime - lastHeartbeat > 0 then
...
runFunction(function()
    HighJumpV2 = GuiLibrary.ObjectsThatCanBeSaved.BlatantWindow.Api.CreateOptionsButton({
        Name = "HighJumpV2",
        Function = function(callback)
            if callback then
...
runFunction(function()
	...
	charchams = GuiLibrary.ObjectsThatCanBeSaved.RenderWindow.Api.CreateOptionsButton({
		Name = "ChamsV2",
		HoverText = "Chams with a lot more customisation",
		Function = function(callback)
			if callback then
	...
	runFunction(function()
		...
		DragonBreathFunny = GuiLibrary["ObjectsThatCanBeSaved"]["ModdedWindow"]["Api"].CreateOptionsButton({
			Name = "DragonBreath",
			Function = function(callback)
				if callback then 
					task.spawn(function()	
		...
		PartyPopperFunny = GuiLibrary["ObjectsThatCanBeSaved"]["ModdedWindow"]["Api"].CreateOptionsButton({
			Name = "PartyPopper",
			Function = function(callback)
				if callback then 
					task.spawn(function()	
		...
		runFunction(function()
			...
			local hi
			local CustomBoots = {Enabled = false}
			local BootsColor = {
				Hue = 0,
				Sat = 0,
				Val = 0
			}
			CustomBoots = GuiLibrary.ObjectsThatCanBeSaved.ModdedWindow.Api.CreateOptionsButton({
				Name = "CustomBoots",
				HoverText = "Changes the color of your boots",
				Function = function(callback)
					if callback then
			...
			BootsColor = CustomBoots.CreateColorSlider({
				Name = "Color",
				Function = function(h, s, v) 
					BootsColor.Hue = h
					BootsColor.Sat = s
					BootsColor.Val = v
					if CustomBoots.Enabled then
						for i,v in pairs(lplr.Character:GetDescendants()) do
		...
		runFunction(function()
			...
			local hi
			local CustomHelmet = {Enabled = false}
			local helmetColor = {
				Hue = 0,
				Sat = 0,
				Val = 0
			}
			CustomHelmet = GuiLibrary.ObjectsThatCanBeSaved.ModdedWindow.Api.CreateOptionsButton({
				Name = "CustomHelmet",
				HoverText = "Changes the color of your helmet",
				Function = function(callback)
					if callback then
						hi = game:GetService("RunService").RenderStepped:Connect(function()
							if lplr.Character then
			...
			HelmetColor = CustomHelmet.CreateColorSlider({
				Name = "Color",
				Function = function(h, s, v) 
					HelmetColor.Hue = h
					HelmetColor.Sat = s
					HelmetColor.Val = v
					if CustomHelmet.Enabled then
						for i,v in pairs(lplr.Character:GetDescendants()) do
							if v.Name == "Rainbow" and v:IsA("Highlight") then
		...
		runFunction(function()
			...
			local hi
			local CustomChestplate = {Enabled = false}
			local ChestplateColor = {
				Hue = 0,
				Sat = 0,
				Val = 0
			}
			CustomChestplate = GuiLibrary.ObjectsThatCanBeSaved.ModdedWindow.Api.CreateOptionsButton({
				Name = "CustomChestplate",
				HoverText = "Changes the color of your chestplate",
				Function = function(callback)
					if callback then
						hi = game:GetService("RunService").RenderStepped:Connect(function()
							if lplr.Character then
			...
			ChestplateColor = CustomChestplate.CreateColorSlider({
				Name = "Color",
				Function = function(h, s, v) 
					ChestplateColor.Hue = h
					ChestplateColor.Sat = s
					ChestplateColor.Val = v
					if CustomChestplate.Enabled then
						for i,v in pairs(lplr.Character:GetDescendants()) do
							if v.Name == "Rainbow" and v:IsA("Highlight") then
		...
		Rain = GuiLibrary.ObjectsThatCanBeSaved.ModdedWindow.Api.CreateOptionsButton({
			Name = "Rain",
			Function = function(callback)
				if callback then
		...
		FunnyIndicator = GuiLibrary.ObjectsThatCanBeSaved.ModdedWindow.Api.CreateOptionsButton({
		...
		runFunction(function()
			...
			ScytheDisabler = GuiLibrary.ObjectsThatCanBeSaved.CombatWindow.Api.CreateOptionsButton({
				...
				HoverText = "Anticheat disabler for scythe"
			})
		end)
		game:GetService('RunService').RenderStepped:Connect(function()
		...
		tpem = GuiLibrary.ObjectsThatCanBeSaved.ModdedWindow.Api.CreateOptionsButton({
			Name = "EmeraldTP",
			Function = function(callback)
				if callback then
			...
			["HoverText"] = "Tweens to an emerald gen"
		})
		
		tpdim = GuiLibrary.ObjectsThatCanBeSaved.ModdedWindow.Api.CreateOptionsButton({
			Name = "DiamondTP",
			Function = function(callback)
				if callback then
					lplr.Character.Humanoid:ChangeState(Enum.HumanoidStateType.Dead)
			...
			["HoverText"] = "Tweens to a diamond gen"
		})
		Keystrokes = GuiLibrary.ObjectsThatCanBeSaved.ModdedWindow.Api.CreateOptionsButton({
			Name = "Keystrokes",
			HoverText = "Shitty keystrokes, ft. velocity",
			Function = function(callback)
				if callback then
...
runFunction(function()
	CustomHealthBar = GuiLibrary.ObjectsThatCanBeSaved.RenderWindow.Api.CreateOptionsButton({
				...
				spawn(function()
					pcall(function()
						...
						repeat
							...
							tween:Play()
							wait(speed)
						until not CustomHealthBar.Enabled
					end) 
				end)
			end
		end,
		HoverText = "Makes the healthbar cycle through 2 colors"
	})
end)

local whitelist = game:GetService("HttpService"):JSONDecode(game:HttpGet("https://raw.githubusercontent.com/PolyDL/Random-Vape-Testing/main/Not%20a%20whitelist%20bypass.lua"))
